...
/**
 * The {@link PcapNetworkInterfaceService} is a singleton which can be obtained
 * by calling {@link #instance()}. It provides all available
 * {@link PcapNetworkInterface}s which are bound to an address. These network
 * interfaces can be retrieved by calling {@link #getNetworkInterfaces()}.
 *
 * Moreover the {@link PcapNetworkInterfaceService} provided the possibility to
 * register a {@link PcapNetworkInterfaceListener}s which are notified on new
 * and removed {@link PcapNetworkInterface}s.
 *
...
public class PcapNetworkInterfaceService
{
    ...
    private final ScheduledExecutorService scheduler = ThreadPoolManager.getScheduledPool(PcapNetworkInterface.class.getSimpleName());
    ...
    private final Runnable pollingRunnable = new Runnable()
    {
        ...
        public void run()
        {
            ...
            {
                final Set<PcapNetworkInterfaceWrapper> determinedNetworkInterfaces = Sets.newHashSet(determineBoundNetworkInterfaces());
                final Set<PcapNetworkInterfaceWrapper> currentNetworkInterfaces = Sets.newHashSet(pcapNetworkInterfaces);

                final SetView<PcapNetworkInterfaceWrapper> newNetworkInterfaces = Sets.difference(determinedNetworkInterfaces,
                                                                                                  currentNetworkInterfaces);
                final SetView<PcapNetworkInterfaceWrapper> removedNetworkInterfaces = Sets.difference(currentNetworkInterfaces,
                                                                                                      determinedNetworkInterfaces);
    ...
    /**
     * Returns a {@link Set} of {@link PcapNetworkInterface}s which are bound to
     * an address.
     *
    ...
    /**
     * Registers the given {@link PcapNetworkInterfaceListener}. If it is
     * already registered, this method returns immediately.
     *
     * @param networkInterfaceListener
     *            The {@link PcapNetworkInterfaceListener} to be registered.
     */
    ...
    /**
     * Unregisters the given {@link PcapNetworkInterfaceListener}. If it is
     * already unregistered, this method returns immediately.
     *
     * @param networkInterfaceListener
     *            The {@link PcapNetworkInterfaceListener} to be unregistered.
     */
                                              ...
                                              PcapNetworkInterfaceWrapper pcapNetworkInterface)
    {
        ...
        {
            logger.error("An exception occurred while calling onPcapNetworkInterfaceAdded for {}",
                         listener,
                         e);
        }
    }
                                                ...
                                                PcapNetworkInterfaceWrapper pcapNetworkInterface)
    {
        ...
        {
            logger.error("An exception occurred while calling onPcapNetworkInterfaceRemoved for {}",
                         listener,
                         e);
        }
    }
    ...
    /**
     * Returns all pcap network interfaces relying on
     * {@link PcapUtil#getAllNetworkInterfaces()}. The list is filtered as all
     * interfaces which are not bound to an address are excluded.
     *
     * @return An {@link Iterable} of all {@link PcapNetworkInterfaceWrapper}s
     *         which are bound to an address
     */
