...
public class PacketCapturingService
{
    ...
    /**
     * Calls {@link #startCapturing(PacketCapturingHandler, String)} with a null
     * MAC address.
     *
     * @param packetCapturingHandler
     *            The handler to be called every time packet is captured
     * @return Returns true, if the capturing has been started successfully,
    ...
    /**
     * Starts the capturing in a dedicated thread, so this method returns
     * immediately. Every time a packet is captured, the
     * {@link PacketCapturingHandler#packetCaptured(MacAddress)} of the given
     * {@link PacketCapturingHandler} is called.
     *
     * It's possible to capture packets sent by a specific MAC address by
     * providing the given parameter. If the macAddress is null, all MAC
     * addresses are considered.
     *
     * @param packetCapturingHandler
     *            The handler to be called every time a packet is captured
     * @param macAddress
     *            The source MAC address of the captured packet, might be null
     *            in order to deactivate this filter criteria
     * @return Returns true, if the capturing has been started successfully,
     *         otherwise returns false
     * @throws IllegalStateException
     *             Thrown if {@link PcapHandle#isOpen()} of {@link #pcapHandle}
     *             returns true
     */
                                  ...
                                  final String macAddress)
    {
        ...
        try
        {
            pcapHandle = pcapNetworkInterface.openLive(SNAPLEN,
        ...
        {
            logger.error("Capturing packets on device {} failed.",
                ...
                try
                {
                    pcapHandle.loop(-1, new PacketListener()
                    {
                        ...
                        public void gotPacket(Packet packet)
                        {
                            ...
                            final MacAddress sourceMacAddress = ethernetPacket.getHeader()
                                                                              .getSrcAddr();
    ...
    /**
     * Stops the capturing. This can be called without calling
     * {@link #startCapturing(PacketCapturingHandler)} or
